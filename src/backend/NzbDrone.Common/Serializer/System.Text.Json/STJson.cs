...
namespace NzbDrone.Common.Serializer;
    public static class STJson
    {
        ...
        where T : new()
        {
            return JsonSerializer.Deserialize<T>(json, SerializerSettings) ?? new T();
        }

        public static object Deserialize(string json, Type type)
        {
            return JsonSerializer.Deserialize(json, type, SerializerSettings) ?? throw new JsonException($"Failed to deserialize {type.Name}");
        }

        public static object Deserialize(Stream input, Type type)
        {
            return JsonSerializer.DeserializeAsync(input, type, SerializerSettings).GetAwaiter().GetResult() ?? throw new JsonException($"Failed to deserialize {type.Name}");
        }
