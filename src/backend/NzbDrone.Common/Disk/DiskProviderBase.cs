...
namespace NzbDrone.Common.Disk;
    ...
    {
        private static readonly ILogger Logger = Log.ForContext<DiskProviderBase>();
        ...
        public virtual string? GetParentFolder(string path)
        {
            Ensure.That(path, () => path).IsValidPath(PathValidationType.CurrentOs);
        ...
        public virtual IMount? GetMount(string path)
        {
            ...
            {
                Log.Debug(ex, "Failed to get mount for path {Path}", path);
                return null;
            }
        }
